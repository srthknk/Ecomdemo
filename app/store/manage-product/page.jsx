...
export default function StoreManageProducts() {
    ...
    const openEditModal = (product) => {
        ...
        if(product.isClothing && product.productSizes) {
            const sizes = {}
            product.productSizes.forEach(ps => {
                sizes[ps.size] = ps.availableUnits
            })
            setStockData(sizes)
        } else {
    ...
    const submitStockUpdate = async () => {
        try {
            ...
            const payload = {
                ...
                isClothing: editingProduct.isClothing,
                stockData
            }
                            ...
                            <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-lg p-4">
                                ...
                                {!isEditingStock ? (
                                    <div className="space-y-2">
                                        {editingProduct?.isClothing ? (
                                            <>
                                                <p className="text-xs text-slate-600 mb-3">Current stock by size:</p>
                                                <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                                                    {editingProduct?.productSizes?.map((ps) => (
                                                        <div key={ps.size} className="bg-white rounded p-2 border border-amber-100 text-center">
                                                            <p className="text-xs font-semibold text-slate-700">{ps.size}</p>
                                                            <p className="text-sm font-bold text-amber-600">{ps.availableUnits}</p>
                                                        </div>
                                                    ))}
                                                </div>
                                            </>
                                    ...
                                    <div className="space-y-3">
                                        {editingProduct?.isClothing ? (
                                            <div className="space-y-3">
                                                <p className="text-xs text-slate-600">Update units for each size:</p>
                                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                                    {editingProduct?.productSizes?.map((ps) => (
                                                        <div key={ps.size}>
                                                            <label className="text-xs font-medium text-slate-700 block mb-1">{ps.size}</label>
                                                            <input
                                                                type="number"
                                                                min="0"
                                                                value={stockData[ps.size] || 0}
                                                                onChange={(e) => handleStockChange(ps.size, e.target.value)}
